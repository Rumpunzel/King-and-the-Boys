...
func move_on_grid(direction_input: Level.Direction) -> void:
	...
	tween.tween_property(model, "position:x", pow(randf_range(-0.5, 0.5), 2.0), animation_duration * 0.75).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
...
func _look_forward(delta: float, direction: Vector3 = velocity) -> void:
	...
	transform = transform.interpolate_with(transform_looking_into_direction, 8.0 * delta)
